...
const Creepers = () => {
    const [CreepersList, setCreepersList] = useState([]); 
    ...
    const fetchPlants = async () => {
        try {
            const response = await axios.post('https://exe201be.io.vn/api/product/search', {
                "pageNum": 1, // Không cần phân trang ở API, lấy tất cả sản phẩm
                "pageSize": 999, // Lấy tất cả sản phẩm, sau đó chia trang thủ công
                "name": debouncedSearchTerm, 
                "status": true,
            });

            if (response.data.success) {
                const filteredPlants = response.data.data.pageData.filter(plant => plant.categoryName === "creepers");
                setCreepersList(filteredPlants);
    ...
    useEffect(() => {
        fetchPlants(); // Gọi API để lấy toàn bộ danh sách Creepers
    }, [debouncedSearchTerm]);
    ...
    const getPaginatedItems = () => {
        ...
        return CreepersList.slice(startIndex, endIndex); // Lấy sản phẩm theo trang
    };
