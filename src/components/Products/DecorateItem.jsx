...
const DecorateItem = () => {
    const [DecorateItemList, setDecorateItemList] = useState([]); 
    ...
    const fetchPlants = async () => {
        try {
            ...
            if (response.data.success) {
                const filteredPlants = response.data.data.pageData.filter(plant => plant.categoryName === "decorateItems");
                setDecorateItemList(filteredPlants);
    ...
    useEffect(() => {
        fetchPlants(); // Gọi API để lấy toàn bộ danh sách DecorateItem
    }, [debouncedSearchTerm]);
    ...
    const getPaginatedItems = () => {
        ...
        return DecorateItemList.slice(startIndex, endIndex); // Lấy sản phẩm theo trang
    };
